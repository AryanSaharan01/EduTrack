...
const verifyToken = (req, res, next) => {
  ...
  if (!authHeader)
    return res
      .status(401)
      .json({ error: "Authentication required", message: "No token provided" });
  const token = authHeader.split(" ")[1];
  if (!token)
    return res
      .status(401)
      .json({ error: "Authentication required", message: "Invalid token format" });
  try {
    ...
    return res
      .status(401)
      .json({ error: "Authentication required", message: "Invalid or expired token" });
  }
};
  ...
  try {
    ...
    const tasks = await pool.query(
      `SELECT DISTINCT
                ...
                CASE 
                    WHEN s.id IS NOT NULL THEN 'graded'
                    ELSE 'published'
                END as status
             FROM lms.tasks t
...
router.get("/:taskId", verifyToken, async (req, res) => {
  try {
    ...
    const questionsResult = await pool.query(
      `SELECT * FROM lms.task_questions 
             WHERE task_id = $1 
